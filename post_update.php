...
$project_ID=mysql_escape_string($_SESSION['projectid']);
...
if($projecttextupdate)
    $sql="INSERT INTO `update`(`project_id`,`data_type`,`data`,`date`) VALUES('$project_ID','text','$projecttextupdate',NOW())";
{
...
&& in_array($extension, $allowedExts))
  ...
  if ($_FILES["updatevideo"]["error"] > 0)
    ...
    if (file_exists("uploads/videos/" . $_FILES["updatevideo"]["name"]))
      ...
      $sql2="INSERT INTO `update`(`project_id`,`data_type`,`data`,`date`) VALUES('$project_ID','video','$target_file',NOW())";
...
|| ($_FILES["updatevideo"]["type"] == "image/png")
...
&& in_array($extension, $allowedExts))
  ...
  if ($_FILES["updatevideo"]["error"] > 0)
    ...
    if (file_exists("uploads/videos/" . $_FILES["updatevideo"]["name"]))
      ...
      $target_file=mysql_real_escape_string($target_file);
      $sql2="INSERT INTO `update`(`project_id`,`data_type`,`data`,`date`) VALUES('$project_ID','image','$target_file',NOW())";
      $connection->query($sql2);
